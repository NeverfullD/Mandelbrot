//       $Id: base.h 41657 2020-11-12 08:32:22Z p20068 $
//      $URL: https://svn01.fh-hagenberg.at/bin/cepheiden/pfc/trunk/pfc/inc/pfc/base.h $
// $Revision: 41657 $
...
template <typename T> constexpr void memcpy (std::span <T> & dst, std::span <T> const & src) noexcept {
   std::memcpy (std::data (dst), std::data (src), std::min (dst.size_bytes (), src.size_bytes ()));
}

template <typename T> constexpr void memset (std::span <T> & span, int const byte) noexcept {
...
template <typename T> auto & read (std::istream & in, std::span <T> & span) noexcept {
   if (in) {
...
template <typename T> auto & write (std::ostream & out, std::span <T> const & span) noexcept {
   if (out) {
