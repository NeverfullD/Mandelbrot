//       $Id: bitmap.h 41683 2020-11-13 12:14:56Z p20068 $
//      $URL: https://svn01.fh-hagenberg.at/bin/cepheiden/pfc/trunk/pfc/inc/pfc/bitmap.h $
// $Revision: 41683 $
//     $Date: 2020-11-13 13:14:56 +0100 (Fr., 13 Nov 2020) $
//   $Author: p20068 $
...
struct bitmap_exception final : std::runtime_error {
   explicit bitmap_exception (std::string const & msg) : std::runtime_error {msg} {
   }
};
...
// using pixel_file_t      = BGR_3_t;
   ...
   static constexpr std::size_t c_size_file_header {sizeof (file_header_t)};
   static constexpr std::size_t c_size_info_header {sizeof (info_header_t)};
   static constexpr std::size_t c_size_pixel       {sizeof (pixel_t)};
   static constexpr std::size_t c_size_pixel_file  {sizeof (pixel_file_t)};
   ...
   public:
      ...
      template <std::integral S> explicit bitmap (S const width, S const height, pixel_span_t pixel_span = {}, bool const clear_span = true) {
     ...
     ~bitmap () = default;
      ...
      template <std::integral S> auto & at (S const x, S const y) noexcept {
         return m_pixel_span[y * m_width + x];
      }

      template <std::integral S> auto const & at (S const x, S const y) const noexcept {
         return m_pixel_span[y * m_width + x];
      }
      ...
      template <std::integral S> void create (S const width, S const height, pixel_span_t pixel_span = {}, bool const clear_span = true) {
         m_width  = std::max <std::size_t> (0, adjust_width (width));
         m_height = std::max <std::size_t> (0, height);

         memset (m_file_header, 0);
         memset (m_info_header, 0);

         m_file_header.type   = 0x4d42;
         m_file_header.size   = c_size_file_header + c_size_info_header + gsl::narrow_cast <dword_t> (size ()) * c_size_pixel_file;
         m_file_header.offset = c_size_file_header + c_size_info_header;

         m_info_header.size       = c_size_info_header;
         m_info_header.width      = gsl::narrow_cast <long_t> (m_width);
         m_info_header.height     = gsl::narrow_cast <long_t> (m_height);
         m_info_header.planes     =  1;
         m_info_header.bit_count  = 24;
         m_info_header.size_image = gsl::narrow_cast <dword_t> (size ()) * c_size_pixel_file;
   ...
   private:
       ...
       */
      static constexpr std::size_t adjust_width (std::size_t const width) noexcept {
         return std::max <std::size_t> (0, ceil_div (gsl::narrow_cast <int> (width * c_size_pixel_file), 4) * 4 / c_size_pixel_file);
      }
      ...
      static bool is_valid (info_header_t const & hdr) {
         return (hdr.bit_count     == 24) &&
                ...
                (hdr.size_image    == gsl::narrow_cast <dword_t> (hdr.width * hdr.height * c_size_pixel_file)) &&
                (hdr.width         == adjust_width (hdr.width));
      }

      static std::size_t pixel_span_from_stream (std::istream & in, pixel_span_t const & pixel_span, std::size_t const chunk_size) noexcept {
//       std::vector <pixel_file_t> buffer      (chunk_size);
      ...
      static std::size_t pixel_span_to_stream (std::ostream & out, pixel_span_t const & pixel_span, std::size_t const chunk_size) noexcept {
//       std::vector <pixel_file_t> buffer      (chunk_size);
         auto                       buffer      {std::make_unique <pixel_file_t []> (chunk_size)};
         auto                       buffer_span {pixel_file_span_t {buffer.get (), chunk_size}};
         auto                       i           {0};
...
class bitmap_group final {
   public:
      template <std::integral S> explicit bitmap_group (S const n, S const width, S const height) : m_group (n) {
         for (auto & bmp : m_group) {
            bmp.create (width, height);
         }
      }

      template <typename G, std::integral S> explicit bitmap_group (G const & memory_group, S const width, S const height) {
...
#endif   // PFC_DETECTED_COMPILER_NVCC
